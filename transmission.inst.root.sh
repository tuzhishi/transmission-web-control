...
sed -i 's/"download-dir.*/"download-dir": "\/home\/'$webuser'\/trdownloads",/g' /etc/transmission-daemon/settings.json
sed -i 's/"encryption.*/"encryption": 2,/g' /etc/transmission-daemon/settings.json
sed -i 's/"rpc-username.*/"rpc-username": "'$webuser'",/g' /etc/transmission-daemon/settings.json
sed -i 's/"rpc-password.*/"rpc-password": "'$webpass'",/g' /etc/transmission-daemon/settings.json
sed -i 's/"rpc-whitelist-enabled.*/"rpc-whitelist-enabled": false,/g' /etc/transmission-daemon/settings.json

mkdir /home/$webuser/trdownloads
chmod -R 775 /home/$webuser/trdownloads
